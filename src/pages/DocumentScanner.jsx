...
import { FileText, Receipt, Landmark, TrendingDown, ShieldAlert, AlertTriangle, Clock, CheckCircle2, Loader2, Trash2, Phone, ChevronUp, ChevronDown } from 'lucide-react';
                ...
                <div className="card-premium rounded-2xl border bg-card overflow-hidden">
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={(e) => deleteBill(bill.id, e)} className="h-7 w-7 rounded-lg hover:bg-destructive/10 flex items-center justify-center shrink-0 ml-2">
                        <Trash2 className="w-3.5 h-3.5 text-muted-foreground" />
                      </button>
                    </div>
                    ...
                    {/* Overcharged line items only */}
                    {overcharges.length > 0 && (
                      <div className="space-y-1.5">
                        <p className="text-xs font-semibold text-destructive flex items-center gap-1.5">
                          <AlertTriangle className="w-3.5 h-3.5" /> {overcharges.length} Overcharge{overcharges.length > 1 ? 's' : ''} Found
                        </p>
                        {overcharges.map((item, idx) => (
                          <div key={idx} className="flex items-center justify-between p-2.5 rounded-xl bg-destructive/5 border border-destructive/10 text-sm">
                            <div className="flex-1 min-w-0">
                              <span className="text-destructive font-medium">{item.description}</span>
                              {item.reason && <p className="text-[11px] text-destructive/70 mt-0.5">{item.reason}</p>}
                            </div>
                            <div className="text-right shrink-0 ml-3">
                              <span className="font-medium">${item.amount?.toFixed(2)}</span>
                              {item.fair_price != null && <p className="text-[11px] text-primary">Fair: ${item.fair_price?.toFixed(2)}</p>}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Call button */}
                    {bill.company_phone && (
                      <a href={`tel:${bill.company_phone}`} className="flex items-center justify-center gap-2 w-full py-2 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors">
